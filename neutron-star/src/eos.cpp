...
double PiecewisePolytropicEoS::returnEnergyDensity(double pressure) const {
    ...
    if (pressure <= P_transition) {
        ...
        double energy_at_transition = P_transition / (Gamma_crust - 1.0);
        double extra_core_energy = (pressure - P_transition) / (Gamma_core - 1.0);
        internal_energy = energy_at_transition + extra_core_energy;
    }
